
<p align="center">
  <a href="https://www.linkedin.com/in/zidane-sheikh-607a1a366">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://github.com/Masterzeidane/masterzeidane">
    <img src="https://img.shields.io/badge/Portfolio-000000?style=flat&logo=vercel&logoColor=white" />
  </a>
  <a href="mailto:cheikhndb575@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white" />
  </a>
</p>

---

## About Me

* 🎓 Final-year Software Engineering student at Taylor's University
* 💻 Building full-stack applications and AI-powered tools
* 🌱 Learning cloud infrastructure, system design, and AWS
* 🚀 Interested in SaaS, automation, and LLM applications
* 🎯 Goal: build production-ready products with real users
* 📫 Reach me at cheikhndb575@gmail.com
## Tech Stack

### Languages

* Python
* JavaScript
* TypeScript
* Java

### Frameworks & Libraries

* React
* Node.js
* Express

### Databases

* PostgreSQL
* MongoDB

### Cloud & DevOps

* AWS
* Docker
* GitHub Actions

## Featured Projects

| Project           | Description                             | Stack                      |
| ----------------- | --------------------------------------- | -------------------------- |
| Portfolio Website | Personal portfolio and project showcase | React, Tailwind            |
| AI Project        | AI-powered application                  | OpenAI API, Node.js        |
| Full Stack App    | End-to-end web application              | React, Express, PostgreSQL |

## GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Masterzeidane&show_icons=true&hide_title=true" width="48%" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Masterzeidane" width="48%" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Masterzeidane&layout=compact" width="40%" />
</p>

---

<p align="center">
Building. Learning. Shipping.
</p>
# masterzeidane-
